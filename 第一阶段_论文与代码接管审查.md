...
> 首次审查日期：2026-08-20  
> 当前更新状态：已完成完整论文阅读、源码静态审查、Gate 0、Gate 1、Gate 2、Gate 3A 和 Gate 3B；Gate 3B 两次 D8 60 epoch 训练严格可重复，独立 validation/test oracle-K 评估通过，总状态为 `PASS_WITH_MORE_EPOCHS_RECOMMENDED`
...
| 尚存运行障碍 | 缺少正式数据/收敛权重；正式规模整库载入内存风险未解决 | 缺少正式数据/收敛权重；中间数据可超过 740 GB，默认 batch 也不适合 16 GB 显存 |
| 当前能否声称复现 | 不能；Gate 1 工程闭环已通过，但尚未训练到收敛或复现论文指标 | 不能，尚未运行且图表证据链不闭合 |
...
|---|---|---|
| 缠绕在脚本中的 `/mnt/data/...` | 已关闭：统一到项目配置和可覆盖 CLI | 第三章 smoke 数据链已运行；正式数据链仍未运行 |
| 硬编码 `cuda:2` | 已关闭：默认并严格校验 `cuda:0` | 第三章 batch 2 单步已测；正式训练峰值显存仍未知 |
...
| 仓库没有正式数据和收敛权重 | 未关闭 | Gate 1 只生成独立 smoke 数据和随机初始化单步 checkpoint，不能视为训练复现 |
...
3. 第四章默认损失包含 Dice，默认 batch 为 192；与已确认的 D8（Focal+offset）、论文描述的 Focal+L1、batch 96 不同。原作者现确认第四章实际训练应关闭 AMP，而论文写启用 AMP；模型分支已定案，但仍没有原始运行命令、日志或 manifest 证明其余完整覆盖参数。
...
建议每讲结束后，不看本文回答对应章节末尾的自检问题。不能回答时回到公式或代码变量，不需要先运行正式实验。Gate 1 的数据审计 JSON 和单步张量记录可作为第 1–3 讲的第一批实物证据；Gate 2 通过目录已经补上第 4–5 讲的坐标、张量和 D8 单步实测证据。
...
2. Gate 1 已在修复后使用新目录完整通过；保留首次失败目录和通过目录，不能把二者混写为同一次运行。
3. Gate 3A 的两次失败证据与修复后通过证据均已保留；中等规模数据、两次 5 epoch 完全确定性、完整 DPD 和时间预算已通过。
4. Gate 3B 已在独立根目录完成；两次 60 epoch 训练严格可重复，冻结 checkpoint 的 validation/test oracle-K 重复评估通过，但 best epoch=57，后续延长训练需另行审批。
5. 下一步优先进入第三章小规模收敛和 oracle K/predicted K 双轨评估方案审批，同时规划第四章延长 epoch 的最小验证；不直接扩展到论文全量参数。
...
- 本项目已生成并独立审计一组隔离的 8/4/4 smoke MAT，并生成仅用于 save/reload 验证的随机初始化单步 checkpoint；尚未获得收敛权重或论文性能结果。第 8.2.8 节记录首次失败，第 8.2.9 节记录修复后通过，两组资源数字不能混用。
- Gate 3A 已生成并审计第四章 1024/256/256 中等规模 IQ 与定位数据，并完成两次 5 epoch D8 pilot 的完全确定性比较；这些 checkpoint 尚未训练到收敛，其 RMSE 不得用作定位性能或论文结果。
- Gate 3A 的 11.665 min 是根据 pilot 稳态 epoch 和样本量比例得到的吞吐外推，不是 60 epoch 实测；只有 Gate 3B 实跑后才能确认墙钟、收敛和独立 test 表现。
