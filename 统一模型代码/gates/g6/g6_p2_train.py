...
from 统一模型代码.gates.g6.g6_p2_model import forward, identity_hits, subband_counts
from 统一模型代码.gates.g6.g6_p2_runtime import ARMS, SEEDS, Progress, read, write, safe_print, g4
...
def summarize(rows):
    ...
    if rows and 'shared_positive' in rows[0]:
        for kind in ('shared','exclusive'):
            total = sum(r[kind+'_positive'] for r in rows)
            result[kind+'_positive'] = total
            for branch in ('semantic','physical'):
                key = f'{branch}_{kind}_missed'
                missed = sum(r[key] for r in rows)
                result[key] = missed
                result[key+'_rate'] = missed/total if total else None
    return result
...
def evaluate(runtime, context, head, arm, bundle, split='val_select', indices=None, label='验证'):
    ...
    try:
        for ids,batch,stats in iterator:
            ...
            weights = head.weights(output[0],output[1],arm).cpu()
            logits = output[1].cpu()
            res = residual.cpu()
            for local,i in enumerate(ids.tolist()):
                ...
                row.update(subband_counts(logits[local],weights[local],targets.band[i],targets.ignore[i],mapping))
...
def band_guard(comparison):
    low,high = comparison['band_only_f1']['ci95']
    return 'MAINTAINED' if low >= -.01 else ('CLEAR_COST' if high < -.01 else 'UNCERTAIN')


def build_report(results, out, training, references):
    ...
    lines = ['# G6-P2运行摘要','','六轨训练及开发集比较完成；test未读取。','',
             '| 比较 | 联合Recall差/pp | GOSPA差/m | RMSE差/m | 判定 |','|---|---:|---:|---:|---|']
    for name,a,b in [('separation_b_minus_a','a','b'),('feedback_c_minus_b','b','c')]:
        ...
        report['paired'][name] = {'combined':combined,'decision':verdict(combined),'band_guard':band_guard(combined),
    ...
    report['frozen_c1_reference'] = {}
    for arm in ARMS:
        pairs = [(references[s],results[s,arm]) for s in SEEDS]
        combined = paired_many(pairs,seed=20260921,repeats=2000)
        combined.update(auxiliary_pairs(pairs,20260921,2000))
        report['frozen_c1_reference'][arm] = {'combined':combined,
            'scope':'Absolute reference with less continuation training; not a causal contrast'}
